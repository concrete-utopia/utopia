...
import { toApiFailure } from '../types'
...
describe('handleListRepoBranches', () => {
  ...
  const fn = async ({ owner, repo, auth }: { owner?: string; repo?: string; auth?: string }) =>
    handleListRepoBranches(newTestRequest({ authCookie: auth }), { owner, repo })

  it('requires a user', async () => {
    await expect(fn({ owner: 'foo', repo: 'bar' })).rejects.toThrow('missing session cookie')
  })
  it('requires a valid owner', async () => {
    await expect(fn({ repo: 'bar', auth: 'bob-key' })).rejects.toThrow('missing owner')
  })
  it('requires a valid repo', async () => {
    await expect(fn({ owner: 'foo', auth: 'bob-key' })).rejects.toThrow('missing repo')
  })
  it('requires a gh auth token', async () => {
    await expect(fn({ owner: 'foo', repo: 'bar', auth: 'bob-key' })).rejects.toThrow('missing auth')
  })
  describe('when the gh call fails', () => {
    it('(generic error) returns the data as a failure', async () => {
      ...
      const got = await fn({ owner: 'foo', repo: 'bar', auth: 'alice-key' })

...
// Copy pasted from Remix: https://github.com/remix-run/remix/blob/main/packages/remix-dev/__tests__/flat-routes-test.ts
...
describe('flatRoutes', () => {
  describe('creates proper route paths', () => {
    let tests: [string, string | undefined][] = [
      ...
      ['(sub).[(sitemap.xml)]', 'sub?/(sitemap.xml)'],
      ['(posts).($slug).([image.jpg])', 'posts?/:slug?/image.jpg?'],
      ['($[$dollabills]).([.]lol).(what).([$]).($up)', ':$dollabills?/.lol?/what?/$?/:up?'],
      ['(sub).(])', 'sub?/]?'],
      ['(sub).([[]])', 'sub?/[]?'],
      ['(sub).([[])', 'sub?/[?'],
      ['(beef])', 'beef]?'],
    ...
    for (let invalid of invalidSlashFiles) {
      test('should error when using `/` in a route segment', () => {
        let regex = new RegExp(/Route segment (".*?") for (".*?") cannot contain "\/"/)
    ...
    let invalidParamFiles: string[] = ['routes/about.[:name].tsx', 'routes/about.:name.tsx']
  ...
  describe("doesn't warn when there's not a route collision", () => {
    let consoleError = jest.spyOn(global.console, 'error').mockImplementation(() => {})
  ...
  describe("warns when there's a route collision", () => {
    ...
    test('folder/route.tsx matching folder.tsx', () => {
      ...
      expect(routes).toHaveLength(1)
      expect(consoleError).toHaveBeenCalledWith(
        getRouteIdConflictErrorMessage(path.posix.join('routes', 'dashboard'), normalizedTestFiles),
      )
    })

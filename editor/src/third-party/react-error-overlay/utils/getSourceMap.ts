...
export class SourceMap {
  ...
  getSources(): string[] {
    return (this.__source_map as any).sources
  }
}

function extractSourceMapUrl(fileUri: string, fileContents: string | null): Promise<string> {
...
export function getSourceMapConsumer(rawSourceMap: any): SourceMap {
  return new SourceMap(new SourceMapConsumer(rawSourceMap))
}
 ...
 */
async function getSourceMap(fileUri: string, fileContents: string | null): Promise<SourceMap> {
  ...
  if (sm.indexOf('data:') === 0) {
    ...
    return new SourceMap(new (SourceMapConsumer as any)(obj))
  }
}

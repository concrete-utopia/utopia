...
export interface NumberOrKeywordInputProps extends InspectorControlProps {
...
export const NumberOrKeywordInput = React.memo<NumberOrKeywordInputProps>(
  (props: NumberOrKeywordInputProps) => {
    ...
    const onSubmitValue: OnSubmitValue<UnknownOrEmptyInput<string>> = React.useCallback(
      (value) => {
        const parsedNewValue = parseInput(value, props.validKeywords)
...
NumberOrKeywordInput.displayName = 'NewNumberOrKeywordControl'

function parseInput(
  value: UnknownOrEmptyInput<string>,
  validKeywords: string[],
): UnknownOrEmptyInput<CSSNumber | CSSKeyword | 'invalid'> {

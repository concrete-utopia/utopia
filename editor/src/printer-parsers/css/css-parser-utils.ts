...
const parseNumber: Parser<CSSNumber> = (value) => {
  if (
    isLexerMatch(value) &&
...
const parseAngle: Parser<CSSNumber> = (value) => {
  if (
    isLexerMatch(value) &&
...
const parseAlphaValue: Parser<CSSNumber> = (value) => {
...
const parseHue: Parser<CSSNumber> = (value) => {
  if (isLexerMatch(value) && value.match[0] != null) {
    return parseAlternative<CSSNumber>(
        ...
        if (isNamedSyntaxType(leaf.syntax, ['rgb()', 'rgba()', 'hsl()', 'hsla()'])) {
          ...
          if (leaf.match[0] != null) {
            ...
            if (isLexerToken(tokenLeaf)) {
              const parsed = parseCSSColor(tokenLeaf.token)
              if (isRight(parsed)) {
                return parsed
              } else {

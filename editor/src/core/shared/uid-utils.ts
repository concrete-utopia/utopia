...
export function setUtopiaID(element: JSXElementChild, uid: string): JSXElementChild {
  if (isUtopiaJSXElement(element)) {
    const newAttrs = setJSXAttributesAttribute(
      element.props,
      'data-uid',
      jsExpressionValue(uid, emptyComments),
      'skip-printing',
    )
    return jsxElement(element.name, uid, newAttrs, element.children)
...
const UID_LENGTH = IS_TEST_ENVIRONMENT ? 3 : PRODUCTION_ENV ? 32 : 4 // in characters

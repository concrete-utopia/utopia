...
describe('shortcuts', () => {
  ...
  describe('x', () => {
    it('when `position: absolute` is set on the selected element, positioning props are removed', async () => {
      ...
      await expectSingleUndoStep(editor, async () => await pressKey('x'))
      await editor.getDispatchFollowUpActionsFinished()
...
describe('global shortcuts to set properties', () => {
  it('cmd + b toggles text to bold', async () => {
    ...
    await expectSingleUndoStep(
      renderResult,
      async () => await pressKey('b', { modifiers: cmdModifier }),
    )
  ...
  it('cmd + i toggles text to italic', async () => {
    ...
    await expectSingleUndoStep(
      renderResult,
      async () => await pressKey('i', { modifiers: cmdModifier }),
    )
    await renderResult.getDispatchFollowUpActionsFinished()
    expect(getPrintedUiJsCode(renderResult.getEditorState())).toEqual(
      makeTestProjectCodeWithSnippet(
  ...
  it('cmd + u toggles text to underline', async () => {
    ...
    await expectSingleUndoStep(
      renderResult,
      async () => await pressKey('u', { modifiers: cmdModifier }),
    )
    await renderResult.getDispatchFollowUpActionsFinished()
    expect(getPrintedUiJsCode(renderResult.getEditorState())).toEqual(
      makeTestProjectCodeWithSnippet(

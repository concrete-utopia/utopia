...
const useColorForDisplayType = (colorTheme: ColorTheme, targetElements: Array<ElementPath>) => {
...
export const FlowReorderDragOutline = controlForStrategyMemoized(
  ({ targetElements }: { targetElements: Array<ElementPath> }) => {
    const scale = useEditorState(
      (store) => store.editor.canvas.scale,
      'FlowReorderDragOutline scale',
    )
    const frame = useEditorState((store) => {
      return getMultiselectBounds(store.strategyState.startingMetadata, targetElements)
    ...
    const color = useColorForDisplayType(colorTheme, targetElements)
...
export const FlowReorderAreaIndicator = controlForStrategyMemoized(
  ({ targetElements }: { targetElements: Array<ElementPath> }) => {
    ...
    const colorTheme = useColorTheme()
    const outlineColor = useColorForDisplayType(colorTheme, targetElements)

    const siblingFramesWithSameDisplayType: CanvasFrameAndTarget[] = useEditorState(
      (store) => {
        if (targetElements.length === 1) {
          const path = targetElements[0]

...
describe('Select Mode Selection', () => {
  ...
  it('keep double clicking on a children eventually selects it – even if it is out of bounds of the parents', async () => {
    ...
    const selectedViews1 = renderResult.getEditorState().unpatchedEditor.selectedViews
    expect(selectedViews1).toEqual([EP.elementPath([[BakedInStoryboardUID, TestSceneUID]])])

    await doubleClick()

    const selectedViews2 = renderResult.getEditorState().unpatchedEditor.selectedViews
    ...
    const selectedViews3 = renderResult.getEditorState().unpatchedEditor.selectedViews
    expect(selectedViews3).toEqual([EP.appendNewElementPath(TestScenePath, ['a'])])

    await doubleClick()

    const selectedViews4 = renderResult.getEditorState().unpatchedEditor.selectedViews
    expect(selectedViews4).toEqual([EP.appendNewElementPath(TestScenePath, ['a', 'b'])])

    await doubleClick()

    const selectedViews5 = renderResult.getEditorState().unpatchedEditor.selectedViews
    expect(selectedViews5).toEqual([EP.appendNewElementPath(TestScenePath, ['a', 'b', 'c'])])

    await doubleClick()

    const selectedViews6 = renderResult.getEditorState().unpatchedEditor.selectedViews
    expect(selectedViews6).toEqual([EP.appendNewElementPath(TestScenePath, ['a', 'b', 'c', 'd'])])

    await doubleClick()

    const selectedViews7 = renderResult.getEditorState().unpatchedEditor.selectedViews
    ...
    const selectedViews8 = renderResult.getEditorState().unpatchedEditor.selectedViews

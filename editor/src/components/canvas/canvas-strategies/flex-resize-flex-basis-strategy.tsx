...
export const flexResizeFlexBasisStrategy: CanvasStrategy = {
  id: 'FLEX_RESIZE_FLEX-BASIS',
  name: 'Flex Resize (flex-basis)',
  isApplicable: (canvasState, interactionState, metadata) => {
    if (canvasState.selectedElements.length === 1) {
      return canvasState.selectedElements.every((element) => {
        const elementMetadata = MetadataUtils.findElementByElementPath(metadata, element)

        // console.log('FlexBasis, isApplicable')

        return (
          elementMetadata?.specialSizeMeasurements.position !== 'absolute' &&
          elementMetadata?.specialSizeMeasurements.parentLayoutSystem === 'flex' &&
          elementMetadata?.specialSizeMeasurements.flexProperties?.hasFlexBasis
        )
      })
    } else {
      return false
  ...
  fitness: (canvasState, interactionState, sessionState) => {
    return flexResizeFlexBasisStrategy.isApplicable(
      canvasState,
      interactionState,

...
describe('Updating a transitive dependency', () => {
  it('Updates the rendered result', async () => {
    const { dispatch, getEditorState, renderedCanvas } = await createAndRenderProject()

    const appRootDivBefore = renderedCanvas.getByTestId('app-root-div')
    ...
    const appRootDivAfter = renderedCanvas.getByTestId('app-root-div')
...
describe('Re-mounting is avoided when', () => {
  ...
  const clickButton = async (editor: EditorRenderResult) => {
    const targetElement = editor.renderedCanvas.getByTestId('clicky')
  ...
  it('arbitrary JS or a component is edited in a regular project', async () => {
    ...
    expect(renderResult.renderedCanvas.queryByText('Clicked 0 times')).not.toBeNull()

    await clickButton(renderResult)

    // Ensure it has been updated
    expect(renderResult.renderedCanvas.queryByText('Clicked 1 times')).not.toBeNull()

...
const navigatorItemWrapperSelector = createCachedSelector(
  ...
  (_: EditorStore, templatePath: TemplatePath) => templatePath,
  (
    jsxMetadataKILLME,
    transientState,
    navigatorTargets,
    ...
    if (isInNavigatorTargets) {
      noOfChildrenInner = MetadataUtils.getImmediateChildren(jsxMetadataKILLME, templatePath).length
    ...
    const elementWarningsInner = getValueFromComplexMap(TP.toString, elementWarnings, templatePath)
...
)((store, templatePath: TemplatePath) => TP.toString(templatePath))
...
export const NavigatorItemWrapper: React.FunctionComponent<NavigatorItemWrapperProps> = betterReactMemo(
  ...
  (props) => {
    const {
      ...
      (s) => navigatorItemWrapperSelector(s, props.templatePath),
      'NavigatorItemWrapper',
    )

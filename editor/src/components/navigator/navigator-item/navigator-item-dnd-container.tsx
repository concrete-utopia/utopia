...
function canDrop(
  ...
  const parentTarget = dropTargetHintType === 'reparent' ? moveToEntry : EP.parentPath(moveToEntry)
  ...
  return notSelectedItem && targetSupportsChildren && targetNotRootOfInstance
}
...
function onHoverDropTargetLine(
  ...
  if (
    targetEntryWithReparentWiggle != null &&
    canDrop(editor, targetEntryWithReparentWiggle.entry.elementPath, 'reparent')
  ) {
    return propsOfDraggedItem.editorDispatch([
      ...targetAction,
      showNavigatorDropTargetHint(
...
function beginDrag(
  ...
  if (!props.selected) {
    props.editorDispatch(
      [
        ...MetaActions.selectComponents([props.elementPath], false),
        showNavigatorDropTargetHint(
          'after',
...
export const NavigatorItemContainer = React.memo((props: NavigatorItemDragAndDropWrapperProps) => {
  ...
  const displayAtEntry = useEditorState(
    Substores.navigator,
    (store) => store.editor.navigator.dropTargetHint.displayAtEntry,
    'NavigatorItemDndWrapper displayAtEntry',
  )

  const dropTargetHintType: DropTargetType = useEditorState(
    Substores.navigator,
  ...
  >(
    () => ({
      ...
      canDrop: (item: NavigatorItemDragAndDropWrapperProps) => {
        ...
        return canDrop(editorStateRef.current, props.elementPath, hintType)
      },
    }),
    [props, moveToEntry, dropTargetHintType],
  )
  ...
  const shouldShowTopHint =
    dropTargetHintType === 'before' &&
    displayAtEntry != null &&
    EP.pathsEqual(props.elementPath, displayAtEntry?.elementPath)
  ...
  const shouldShowBottomHint =
    (dropTargetHintType === 'after' || dropTargetHintType === 'reparent') &&
    displayAtEntry != null &&
    EP.pathsEqual(props.elementPath, displayAtEntry?.elementPath)
